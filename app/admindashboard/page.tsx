...
export default function DashboardPage() {
  ...
  const getStatusText = (status: string) => {
    // Normalize backend status strings to the i18n keys defined in I18nProvider
    ...
    return t(`tickets.${key}` as const)
  }

  const getPriorityText = (priority: string) => {
    const priorityKey = `tickets.${priority}` as const
    ...
    return (
      <div className="bg-white rounded-lg shadow-lg p-6">
        <p className="text-red-600">
          {t('messages.error.generic')}: {(error as any)?.message ?? t('messages.error.loadingDashboard')}
        </p>
        <button
          onClick={() => refetch()}
